...
sh /Applications/SonarQube/bin/macosx-universal-64/sonar.sh consoles
...
rm -rf Report/sonar-reports
mkdir Report/sonar-reports
...
if [ "$unittests" = "on" ]; then
    ...
    echo "<?xml version='1.0' encoding='UTF-8' standalone='yes'?><testsuites name='AllTestUnits'></testsuites>" > Report/sonar-reports/test-report.xml
    echo "<?xml version='1.0' ?><!DOCTYPE coverage SYSTEM 'http://cobertura.sourceforge.net/xml/coverage-03.dtd'><coverage><sources></sources><packages></packages></coverage>" > Report/sonar-reports/coverage-swift.xml
    ...
    runCommand  Report/sonar-reports/xcodebuild.log "${buildCmd[@]}"
    cat Report/sonar-reports/xcodebuild.log  | $XCPRETTY_CMD -t --report junit
    mv build/reports/junit.xml Report/sonar-reports/TEST-report.xml
    ...
    slatherCmd+=( --input-format profdata $excludedCommandLineFlags --cobertura-xml --output-directory Report/sonar-reports)
    ...
    mv Report/sonar-reports/cobertura.xml Report/sonar-reports/coverage-swift.xml
fi
...
if [ "$swiftlint" = "on" ]; then
	if hash $SWIFTLINT_CMD 2>/dev/null; then
		...
		while read word; do
		    ...
		    $SWIFTLINT_CMD lint --path "$word" > Report/sonar-reports/"$appScheme"-swiftlint.txt
...
if [ "$tailor" = "on" ]; then
	if hash $TAILOR_CMD 2>/dev/null; then
		...
		while read word; do
			  ...
			  # Run tailor command
		    $TAILOR_CMD $tailorConfiguration "$word" > Report/sonar-reports/"$appScheme"-tailor.txt
...
if [ "$oclint" = "on" ] && [ "$hasObjC" = "yes" ]; then
	...
	while read word; do
	    ...
	    runCommand no oclint-json-compilation-database -v $includedCommandLineFlags -- -rc LONG_LINE=$longLineThreshold -max-priority-1 $maxPriority -max-priority-2 $maxPriority -max-priority-3 $maxPriority -report-type pmd -o Report/sonar-reports/$appScheme-oclint.xml
...
if [ "$fauxpas" = "on" ] && [ "$hasObjC" = "yes" ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        then
            ...
            fauxpas -o json check $projectFile --workspace $workspaceFile --scheme $appScheme > Report/sonar-reports/fauxpas.json
            ...
            while read projectName; do
                ...
                then
                    ...
                    then
                        ...
                        fauxpas -o json check $projectName --workspace $workspaceRelativePath --scheme $scheme > Report/sonar-reports/$(basename $projectName .xcodeproj)-$scheme-fauxpas.json
                    fi
                fi
...
if [ "$lizard" = "on" ]; then
	if hash $LIZARD_CMD 2>/dev/null; then
		echo -n 'Running Lizard...'
  		$LIZARD_CMD --xml "$srcDirs" > Report/sonar-reports/lizard-report.xml
  	else
...
if [ "$sonarscanner" = "on" ]; then
    ...
    if hash /dev/stdout /Applications/SonarScanner/bin/sonar-scanner
 2>/dev/null; then
        runCommand /dev/stdout /Applications/SonarScanner/bin/sonar-scanner
 $numVersionSonarRunner
    else
    ...
    if hash /dev/stdout sonar-runner 2>/dev/null; then
	   ...
	   runCommand /dev/stdout /Applications/SonarScanner/bin/sonar-scanner $numVersionSonarRunner
    fi
fi
